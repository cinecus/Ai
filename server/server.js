...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null, UPLOAD_DIR); // Save files in 'uploads' folder
    },
    filename: (req, file, cb) => {
        const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9);
        cb(null, uniqueSuffix + '-' + file.originalname); // Create unique filenames
    }
});
        ...
        if (game) {
            ...
            MongoClient.connect(url, function (err, db) {
                ...
                dbo.collection("kahootGames").find(query).toArray(function (err, res) {
                    ...
                    const urlImage = res[0].questions[0].urlImage;
        ...
        MongoClient.connect(url, function (err, db) {
            ...
            dbo.collection("kahootGames").find(query).toArray(function (err, res) {
                ...
                if (res[0].questions.length >= game.gameData.question) {
                    ...
                    const urlImage = res[0].questions[questionNum].urlImage;
